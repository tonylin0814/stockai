...
export async function POST() {
  ...
  try {
    ...
    for (const division of ["gpt", "anthropic"] as Division[]) {
      ...
      await supabase.from("sim_daily_reports").upsert(
        {
          ...
          us_day_pnl_pct: usPortfolio ? ((usValue - Number(usPortfolio.starting_cash)) / Number(usPortfolio.starting_cash)) * 100 : null,
          tw_day_pnl_pct: twPortfolio ? ((twValue - Number(twPortfolio.starting_cash)) / Number(twPortfolio.starting_cash)) * 100 : null,
      ...
      await saveExtractedPredictions({ supabase, userId: user.id, division, reportDate, predictions });
