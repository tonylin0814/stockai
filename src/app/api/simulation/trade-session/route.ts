...
async function ensureConfig(supabase: ReturnType<typeof createSupabaseServiceClient>, userId: string) {
...
async function executeTrade(params: {
  ...
  if (await hasSameDayConflict(params.supabase, params.portfolio.id, decision.symbol, decision.action, params.sessionDate)) {
    return false;
  }
  ...
  if (sharesToSell >= Number(existing.shares)) {
    await params.supabase
      .from("sim_positions")
      .update({ status: "closed", closed_at: new Date().toISOString(), current_price: params.quote.price })
      .eq("id", existing.id);
  } else {
    await params.supabase
      .from("sim_positions")
...
export async function POST() {
  ...
  try {
    const config = await ensureConfig(supabase, user.id);

    for (const division of divisions) {
      for (const market of markets) {
        if (!isWithinTradingHours(config, market)) {
          summary.push(`${division}/${market} 不在交易時段。`);
          continue;
        }

        const portfolio = await ensurePortfolio(supabase, user.id, division, market);
        ...
        const candidates = await loadCandidates(supabase, user.id, market);
        ...
        const vixQuote =
          market === "US"
            ? await getMarketDataProvider().getIndex("^VIX", "US").catch(() => null)
            : null;
    ...
    return NextResponse.json({ message: summary.join(" ") || "今日交易完成。" });
  } catch (error) {
